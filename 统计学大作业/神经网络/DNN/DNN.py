...
model.compile(optimizer='adam', loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
              metrics=['accuracy'])
test_loss, test_acc = model.evaluate(feature_test, target_test, verbose=2)
history = model.fit(feature_train, target_train, validation_data=(feature_test, target_test), epochs=15)
model.summary()
plt.clf()
